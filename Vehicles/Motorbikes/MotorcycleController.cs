...
public class MotorcycleController : MonoBehaviour 
{
    ...
    wheels[0] = new WheelData(frontWheelMesh, frontWheelCollider);
    wheels[1] = new WheelData(backWheelMesh, backWheelCollider);
	...
	private void motoMove(MotoInput input) {
		frontWheelCollider.steerAngle = Mathf.Clamp(input.steer, -1, 1) * maxSteerAngle;

		frontWheelCollider.brakeTorque = maxForwardBrake * input.brakeForward;
		backWheelCollider.brakeTorque = maxBackBrake * input.brakeBack;

		backWheelCollider.motorTorque = maxMotorTorque * input.acceleration;
	}
